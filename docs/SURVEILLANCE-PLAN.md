...
### ⑤ 全画面の直感的・100%日本語ローカライズ (Intuitive UI Localization)
全UI（Classic UI、Observatory 3D画面、設定パネル、エラーメッセージ等）の未翻訳部を完全抽出し、初心者でも直感で理解できる日本語表記へ統合：
- 単なる直訳ではなく補足付き親切表記（例: `Empty Room` ➔ `空部屋測定（ベースライン校正）`, `CSI Variance` ➔ `電波変動量（動作強度）`, `Fall Detect` ➔ `転倒検知アラート`）
- 100%完全な言語切り替え・一元管理
...
3. 全画面の直感的・100%日本語ローカライズ一括適用
4. 本家 (`upstream/main`) からの最新バグ改修マージと同期
